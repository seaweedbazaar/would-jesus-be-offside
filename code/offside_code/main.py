...
SAMPLE_SPACING_CELL_FRACTION = 0.055    # trace horizontal density, as a
                                       # fraction of cell_size (so it
                                       # scales with resolution instead
                                       # of being a fixed pixel count --
                                       # otherwise the wave's proportions
                                       # break on a different-resolution
                                       # screen than the one it was tuned on)
TRACE_COLOR = (180, 255, 215)  # brighter mint-green, more contrast on weak screens
TRACE_LINE_THICKNESS = 3        # waveform line thickness in pixels
BG_COLOR = (5, 8, 6)
GRID_COLOR = (55, 95, 78)       # thin gridlines
GRID_COLOR_THICK = (78, 124, 103)  # emphasized gridlines -- clearly brighter than thin ones

# --- panel layout: 14 rows tall, fills the whole screen ---
PANEL_ROWS = 14
PANEL_HEIGHT_RATIO = 0.82        # panel fills the entire screen height
PANEL_WIDTH_RATIO = 0.98         # panel fills the entire screen width
...
def compute_panel(w, h):
    ...
    # round (not floor) to the nearest whole number of columns, then force
    # the panel to exactly fill the available width -- this makes columns
    # *almost* square (off by a negligible fraction of a pixel) instead of
    # leaving a visible leftover gap on the left/right from int() flooring
    columns = max(1, round(raw_width / cell_size))
    panel_width = raw_width
    panel_x = (w - panel_width) / 2

    panel_y_top = (h - panel_height) / 2
    panel_y_bottom = panel_y_top + panel_height
...
def draw_thick_aalines(surface, color, points, thickness):
    """pygame's antialiased line drawing (aalines) is always 1px wide with
    no width parameter. To get a thicker line while keeping the smooth
    antialiased look, draw it multiple times at small vertical offsets."""
    if thickness <= 1:
    ...
    for k in range(n_passes):
        offsets.append(-half + (k + 0.5) * (thickness / n_passes))
    for dy in offsets:
...
def draw_panel_grid(surface, panel_x, panel_y_top, panel_width, panel_height, cell_size, columns, panel_y_bottom):
    ...
    for i in range(PANEL_ROWS + 1):
        y = round(panel_y_bottom - i * cell_size)
        thick = i in THICK_ROW_STEPS
        color = GRID_COLOR_THICK if thick else GRID_COLOR
        width = 2 if thick else 1
        pygame.draw.line(surface, color, (round(panel_x), y), (round(panel_x + panel_width), y), width)

    # vertical lines: columns + 1 lines, j = 0 (left) .. columns (right)
    col_width = panel_width / columns
    for j in range(columns + 1):
        x = round(panel_x + j * col_width)
        thick = (j % THICK_COL_STEP == 0)
        color = GRID_COLOR_THICK if thick else GRID_COLOR
        width = 2 if thick else 1
        pygame.draw.line(surface, color, (x, round(panel_y_top)), (x, round(panel_y_top + panel_height)), width)
...
def main():
    ...
    while running:
        ...
        for i, y in enumerate(buffer):
            ...
            points.append((x, py))
        if len(points) > 1:
            draw_thick_aalines(screen, TRACE_COLOR, points, TRACE_LINE_THICKNESS)
